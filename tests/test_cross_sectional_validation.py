...
class TestCrossSectionalValidation(unittest.TestCase):
    ...
    def test_benchmark_relative_verdict_requires_active_edge(self):
        self.assertEqual(validation_verdict(1.60, 1.20, folds=8), VERDICT_EDGE)
        self.assertEqual(validation_verdict(1.00, 1.26, folds=8), VERDICT_FAILS)
        self.assertNotEqual(validation_verdict(1.00, 1.26, folds=8), VERDICT_EDGE)
        self.assertEqual(validation_verdict(-0.10, -0.30, folds=12), VERDICT_FAILS)
